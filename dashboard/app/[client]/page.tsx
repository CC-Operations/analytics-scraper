...
const TYPE_COLORS: Record<string, string> = {
  ...
  Sidecar: "bg-white/5 text-white/60 border border-white/10",
...
function ViewsChart({ data }: { data: Post[] }) {
  ...
  if (chartData.length === 0)
    return <div className="flex items-center justify-center h-40 text-white/20 text-sm">No data yet</div>;
...
function PostsTable({ posts, onToggle }: { posts: Post[]; onToggle: (id: number, excluded: boolean) => void }) {
  if (posts.length === 0)
    return <div className="flex items-center justify-center h-48 text-white/20">No posts yet for this platform</div>;
        ...
        {posts.map((post, i) => (
          <tr
            ...
            className={`border-b border-white/[0.04] transition-all ${
              post.excluded ? "opacity-25" : "hover:bg-white/[0.02]"
            } ${i === posts.length - 1 ? "border-0" : ""}`}
          >
            <td className="px-4 py-3 text-center">
              <Toggle checked={!post.excluded} onChange={() => onToggle(post.id, !post.excluded)} />
            </td>
            <td className="px-4 py-3 text-white/40 whitespace-nowrap font-mono text-xs">{post.posted_date ? post.posted_date.slice(0, 10) : "—"}</td>
            ...
            <td className="px-4 py-3 text-center">
              {post.post_url ? (
                <a href={post.post_url} target="_blank" rel="noopener noreferrer"
                  className="text-white/20 hover:text-white transition-colors">↗</a>
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-10">
          <div>
            <span className="text-xs uppercase tracking-widest font-medium mb-2 block"
              style={{ color: PINK }}>Creator Camp</span>
        ...
        <div className="flex gap-1 mb-8">
          {CLIENTS.map((c) => {
            const isActive = c.toLowerCase() === client;
            return (
            ...
            return (
              <button key={p} onClick={() => !isComing && setActivePlatform(p)} disabled={isComing}
                ...
                style={isActive
                  ...
                  : isComing
                  ? { borderColor: "transparent", backgroundColor: "transparent", color: "rgba(255,255,255,0.15)", cursor: "not-allowed" }
                  : { borderColor: "rgba(255,255,255,0.06)", backgroundColor: "transparent", color: "rgba(255,255,255,0.4)" }
                }
              >
                {p !== "Overview" && (
          ...
          <>
            ...
            {activePlatform === "Overview" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6">
                  <p className="text-white/40 text-xs uppercase tracking-widest mb-5 font-medium">Views & Likes Over Time</p>
                  <ViewsChart data={counted} />
                </div>
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6">
                  <p className="text-white/40 text-xs uppercase tracking-widest mb-5 font-medium">Performance by Platform</p>
            ...
            {activePlatform !== "Overview" && counted.length > 0 && (
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6 mb-8">
                <p className="text-white/40 text-xs uppercase tracking-widest mb-5 font-medium">{activePlatform} — Views Over Time</p>
                <ViewsChart data={counted} />
              </div>
            )}
